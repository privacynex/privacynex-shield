...
export function normalizePolicyVersion(input) {
...
function randomNonce(maxNumber = MAX_NUMBER) {
...
function networkPrefix(request) {
...
async function buildChallengeBinding(request, secret) {
...
export async function verifyPoW(salt, nonce, target) {
...
export async function generateSignedChallenge(request, secret, policyVersion = DEFAULT_POLICY_VERSION, maxNumber = MAX_NUMBER) {
...
export async function verifySignedChallenge(challengeToken, request, secret, policyVersion = DEFAULT_POLICY_VERSION) {
    try {
        const pv = normalizePolicyVersion(policyVersion);
        ...
        if (!/^\d{1,12}$/.test(tsStr) || !/^\d{1,7}$/.test(maxStr))
            return null;
        const ts = Number(tsStr);
        const max = Number(maxStr);
        if (!Number.isSafeInteger(ts) || !Number.isSafeInteger(max))
            return null;
        if (max < 1000 || max > MAX_POW_NUMBER)
            return null;
        if (!JTI_RE.test(jti) || !SALT_RE.test(salt) || !HASH_RE.test(target))
            return null;
        if (!HASH_RE.test(binding) || !HASH_RE.test(hmacSig))
            return null;
        // Policy version match required : bumping env var invalidates all tokens
        if (tokenPv !== pv)
            return null;
        const now = Math.floor(Date.now() / 1000);
        if (ts > now + CLOCK_SKEW_SECONDS || now - ts > CHALLENGE_TTL)
            return null;
...
export async function generateSignedCookie(powHash, request, secret, policyVersion = DEFAULT_POLICY_VERSION) {
...
export async function verifySignedCookie(cookieValue, request, secret, policyVersion = DEFAULT_POLICY_VERSION) {
    try {
        const pv = normalizePolicyVersion(policyVersion);
        ...
        if (!Number.isSafeInteger(ts))
            return false;
        if (!HASH_RE.test(powHash) || !HASH_RE.test(binding) || !HASH_RE.test(hmacSig))
            return false;
        ...
        // Expiry check
        const now = Math.floor(Date.now() / 1000);
        if (ts > now + CLOCK_SKEW_SECONDS || now - ts > COOKIE_TTL)
            return false;
        const expectedBinding = await buildChallengeBinding(request, secret);
        if (binding !== expectedBinding)
            return false;
