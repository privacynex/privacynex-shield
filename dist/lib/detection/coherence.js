...
// ─── Helpers ───
function extractSecChUaPlatform(secChUaPlatform) {
    if (!secChUaPlatform)
        return '';
    const cleaned = secChUaPlatform.replace(/^"+|"+$/g, '').trim();
    return cleaned.toLowerCase();
}
function extractSecChUaMobile(secChUaMobile) {
    if (!secChUaMobile)
        return null;
...
function checkPlatform(ua, secChUaPlatform) {
...
function checkLangCountry(acceptLang, country) {
    if (!acceptLang || !country || country === 'XX' || country === 'T1')
        return null;
    ...
    const languageToExpectedCountries = {
        'zh': ['CN', 'TW', 'HK', 'SG', 'MO'],
        'ja': ['JP'],
        'ko': ['KR', 'KP'],
...
function checkUaOsStale(ua) {
    ...
    if (firefoxMatch) {
        ...
        if (v < 100)
            return 'coherence:ua-os-stale:firefox-old';
    }
    return null;
}
 ...
 */
export function scoreCoherence(request) {
    const signals = [];
    let weight = 0;
