...
import { generateSignedChallenge, verifySignedCookie } from "../core/crypto.js";
import { decideMultiLevel, getDecisionMode } from "../decision/engine.js";
import { detectBlockedBotUA, isContentBypassRequest, isKeepInternetWorkingPath, isTrustedShieldBypassRequest, } from "../detection/bot-policy.js";
import { scoreCoherence } from "../detection/coherence.js";
import { checkRateLimit } from "./ratelimit.js";
import { isShieldDocumentRequest, isShieldDocumentResponse } from "./request.js";
import { buildBadAsns, pickPowMax, scoreRequest } from "../scoring/score.js";
function resolveRateLimit(request, bucket) {
...
function challengePage(env, challenge) {
 ...
 */
export async function shieldFetch(request, env, next) {
    configureClientIpHeader(env.SHIELD_CLIENT_IP_HEADER);
    if (env.SHIELD_ENABLED === 'false')
        return next();
    if (!env.SHIELD_SECRET)
        return unavailableResponse();
    const url = new URL(request.url);
    const path = url.pathname;
    if (isKeepInternetWorkingPath(path))
        return next();
    const blockedBot = detectBlockedBotUA(request.headers.get('User-Agent') || '');
    if (blockedBot)
        return blockedResponse();
    if (path.startsWith('/api/')) {
        if (resolveRateLimit(request, 'api').limited)
            return blockedResponse();
        return next();
    }
    ...
    if (!isShieldDocumentResponse(request, path, downstream))
        return downstream;
    if (resolveRateLimit(request, 'document').limited)
        return blockedResponse();
    ...
    if (decision.action === 'deny')
        return blockedResponse();
    if (isTrustedShieldBypassRequest(request, path)
    ...
    try {
        const challenge = await generateSignedChallenge(request, env.SHIELD_SECRET, env.SHIELD_POLICY_VERSION, max);
