...
import {
  DENY_SIGNALS,
  HARD_CHALLENGE_SIGNALS,
  SOFT_CHALLENGE_SIGNALS,
  LOG_ONLY_SIGNALS,
  SENSITIVE_PATHS,
  ADMIN_PATHS,
...
const VALID_MODES: readonly ShieldDecisionMode[] = ['legacy', 'shadow', 'multi'];

export function getDecisionMode(env: DecisionEnv | undefined): ShieldDecisionMode {
  if (!env || typeof env.SHIELD_DECISION_MODE !== 'string') return SHIELD_DECISION_MODE_DEFAULT;
  const raw = env.SHIELD_DECISION_MODE.trim().toLowerCase();
  for (let i = 0; i < VALID_MODES.length; i++) {
    if (VALID_MODES[i] === raw) return VALID_MODES[i] as ShieldDecisionMode;
  }
...
function signalsContainAny(signals: readonly string[], targets: readonly string[]): boolean {
  for (let i = 0; i < signals.length; i++) {
    const s = signals[i];
    for (let j = 0; j < targets.length; j++) {
      if (s === targets[j] || s.startsWith(targets[j])) return true;
    }
  }
  return false;
}
...
export function decideMultiLevel(input: DecisionInput): { action: ShieldAction; reason: string } {
