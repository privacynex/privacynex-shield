...
import { SHIELD_BOT_IP_SNAPSHOT } from './bot-ip-snapshot.ts';
...
const TRUSTED_CRAWLERS: readonly TrustedCrawlerRule[] = Object.freeze([
  {
    label: 'Googlebot',
...
const DENY_UA_PATTERNS: readonly string[] = Object.freeze([
...
export const DENY_SIGNALS: readonly string[] = Object.freeze([
...
export const HARD_CHALLENGE_SIGNALS: readonly string[] = Object.freeze([
  'playwright', 'phantomjs', 'webdriver', 'cdp:automation',
  'selenium:cdc', 'selenium:globals', 'puppeteer',
]);

export const SOFT_CHALLENGE_SIGNALS: readonly string[] = Object.freeze([
  'headless:dims', 'coherence:platform-mismatch',
  'coherence:mobile-mismatch', 'coherence:lang-country-mismatch',
]);

export const LOG_ONLY_SIGNALS: readonly string[] = Object.freeze([
...
export const ADMIN_PATHS: readonly string[] = Object.freeze([
...
function trustedRuleFor(ua: string): TrustedCrawlerRule | null {
...
function isCrawlerPathAllowed(path: string): boolean {
  const normalized = path.toLowerCase();
  const segments = normalized.split('/').filter(Boolean);
  if (segments.some((segment) => segment.startsWith('.'))) return false;
  if (segments.some((segment) => segment === 'admin' || segment === 'api')) return false;
...
interface CompiledCidr {
  ...
  readonly maskOrShift: number | bigint;
}

function compileCidr(cidr: string): CompiledCidr | null {
  const [base, prefixRaw] = cidr.split('/');
  const prefix = Number(prefixRaw);

  if (base.includes(':')) {
    const baseValue = parseIpv6(base);
    if (baseValue === null || !Number.isInteger(prefix) || prefix < 0 || prefix > 128) return null;
    const shift = 128n - BigInt(prefix);
    return { family: 6, base: baseValue >> shift, maskOrShift: shift };
  }

  const baseValue = parseIpv4(base);
  if (baseValue === null || !Number.isInteger(prefix) || prefix < 0 || prefix > 32) return null;
  const mask = prefix === 0 ? 0 : (0xffffffff << (32 - prefix)) >>> 0;
  return { family: 4, base: baseValue & mask, maskOrShift: mask };
}

function authoritativeClientIp(request: Request): string {
  return getClientIp(request);
}

const COMPILED_BOT_IP_SNAPSHOT = new Map<string, readonly CompiledCidr[]>(
  Object.entries(SHIELD_BOT_IP_SNAPSHOT).map(([url, prefixes]) => [
    url,
    Object.freeze(prefixes.map(compileCidr).filter((value): value is CompiledCidr => value !== null)),
  ]),
);
const VERIFICATION_CACHE_MAX = 512;
const VERIFICATION_CACHE_TTL_MS = 300_000;
const verificationCache = new Map<string, { readonly expiresAt: number; readonly verified: boolean }>();

function matchesCompiledIp(ip: string, prefix: CompiledCidr): boolean {
  if (prefix.family === 4) {
    const value = parseIpv4(ip);
    const mask = prefix.maskOrShift as number;
    return value !== null && (value & mask) === prefix.base;
  }
  const value = parseIpv6(ip);
  const shift = prefix.maskOrShift as bigint;
  return value !== null && (value >> shift) === prefix.base;
}

function isRuleVerified(request: Request, rule: TrustedCrawlerRule): boolean {
  ...
  if (verificationCache.size >= VERIFICATION_CACHE_MAX) {
    ...
    if (!oldest.done) verificationCache.delete(oldest.value);
  }
  verificationCache.set(cacheKey, { expiresAt: now + VERIFICATION_CACHE_TTL_MS, verified });
  return verified;
}

export function detectBlockedBotUA(ua: string): string | null {
  if (!ua || trustedRuleFor(ua)) return null;
  return DENY_UA_PATTERNS.find((pattern) => ua.toLowerCase().includes(pattern.toLowerCase())) || null;
}

export function isTrustedShieldBypassRequest(request: Request, path = new URL(request.url).pathname): boolean {
  if (!isCrawlerPathAllowed(path)) return false;
  const rule = trustedRuleFor(request.headers.get('User-Agent') || '');
  return Boolean(rule && rule.scope === 'full' && isRuleVerified(request, rule));
}

export function isContentBypassRequest(request: Request, path = new URL(request.url).pathname): boolean {
  if (!isCrawlerPathAllowed(path)) return false;
  const rule = trustedRuleFor(request.headers.get('User-Agent') || '');
  return Boolean(rule && rule.scope === 'content' && isRuleVerified(request, rule));
}

export function isKeepInternetWorkingPath(path: string): boolean {
...
export function listTrustedCrawlerRules(): readonly TrustedCrawlerRule[] {
