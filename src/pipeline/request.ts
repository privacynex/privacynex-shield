 ...
 */
export function isShieldDocumentRequest(request: Request, path: string): boolean {
  if (request.method !== 'GET') return false;
  if (path === '/api' || path.startsWith('/api/')) return false;
  return true;
}

export function isShieldDocumentResponse(
  request: Request,
  path: string,
  response: Response,
): boolean {
  if (!isShieldDocumentRequest(request, path)) return false;
 ...
 */
export function getShieldDocumentDisposition(
  action: ShieldDocumentAction,
  hasValidCookie: boolean,
): ShieldDocumentDisposition {
