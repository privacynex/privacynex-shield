import { configureClientIpHeader } from '../config/runtime.ts';
import { generateSignedChallenge } from '../core/crypto.ts';
...
export function handleShieldChallengeOptions(request: Request): Response {
...
export async function handleShieldChallenge(request: Request, env: ChallengeEnv): Promise<Response> {
  configureClientIpHeader(env.SHIELD_CLIENT_IP_HEADER);
  if (request.method !== 'GET' || !isAllowedFetchContext(request)) return returnInternal404(request);
