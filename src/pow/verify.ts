...
import { CHALLENGE_TTL, COOKIE_TTL, MAX_POW_NUMBER, generateSignedCookie, verifyPoW, verifySignedChallenge } from '../core/crypto.ts';
import { getInternalCorsHeaders, isAllowedFetchContext, isAllowedOriginOrReferer, isSuspiciousBot, returnInternal404 } from '../http/internal-api.ts';
import { readBoundedJson, RequestBodyError } from '../http/request-body.ts';
import { checkRateLimit, RATE_LIMIT_CONFIG } from '../pipeline/ratelimit.ts';
import { markSpentOrReject } from '../pipeline/replay.ts';
...
function isValidNonce(value: unknown): value is number {
  return typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= MAX_POW_NUMBER;
}

function isValidChallengeToken(value: unknown): value is string {
  return typeof value === 'string' && value.length >= 128 && value.length <= 1024;
}

function verifyRateLimit(request: Request): 'ok' | 'limited' {
  const ip = getClientIp(request) || 'unknown';
  return checkRateLimit(ip, 'verify').limited ? 'limited' : 'ok';
}

function cookieHeaders(request: Request, value: string): string[] {
...
export function handleShieldVerifyOptions(request: Request): Response {
  if (!isAllowedFetchContext(request)) return returnInternal404(request);
...
export async function handleShieldVerify(request: Request, env: VerifyEnv): Promise<Response> {
  configureClientIpHeader(env.SHIELD_CLIENT_IP_HEADER);
  if (request.method !== 'POST' || !isAllowedFetchContext(request)) return returnInternal404(request);
  if (isSuspiciousBot(request) && !isAllowedOriginOrReferer(request)) return returnInternal404(request);
  ...
  if (!isValidChallengeToken(challengeToken)) return json(request, { error: 'Missing challenge token' }, 400);
  if (!isValidNonce(nonce)) return json(request, { error: 'Invalid nonce' }, 400);
  ...
  try {
    ...
    if (!verified) return json(request, { error: 'Invalid challenge token' }, 403);
    if (nonce > verified.max) return json(request, { error: 'Nonce outside signed challenge range' }, 403);
    ...
    if (spent.replay) return json(request, { error: 'Challenge already consumed' }, 403);

    const response = json(request, { ok: true, ttl: COOKIE_TTL, challengeTtl: CHALLENGE_TTL }, 200);
    for (const header of cookieHeaders(request, signedCookie)) response.headers.append('Set-Cookie', header);
    return response;
  } catch {
